...
export default function EditRecordPage() {
  ...
  useEffect(() => {
    async function fetchInitialData() {
      ...
      const { data: recordData, error: recordError } = await supabase
        .from('records')
        .select('*, games:games!records_game_id_fkey(*)')
        .eq('id', id)
        .single();

      if (recordError) {
        setError('記録の取得に失敗しました: ' + recordError.message);
        setLoading(false);
        return;
      }
      ...
      if (recordData) {
        const recordGame = recordData.games;
        if (recordGame) {
          setSelectedDate(dayjs(recordGame.date));
          const formattedDate = dayjs(recordGame.date).format('YYYY-MM-DD');
          const gamesOnSameDay = gamesData.filter((game: Game) => game.date === formattedDate);
          setFilteredGames(gamesOnSameDay);
          setSelectedGameId(String(recordData.game_id));
          setDisplayStadium(recordGame.stadium || '');
          setDisplayWinningPitcher(recordGame.winning_pitcher || '');
          setDisplayLosingPitcher(recordGame.losing_pitcher || '');
        }
  ...
  useEffect(() => {
    if (selectedDate) {
        ...
        const gamesOnSameDay = allGames.filter(game => game.date === formattedDate);
        setFilteredGames(gamesOnSameDay);
    } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (error) {
      setError('記録の更新に失敗しました: ' + error.message);
    } else {
      router.refresh();
      router.push('/');
    }
